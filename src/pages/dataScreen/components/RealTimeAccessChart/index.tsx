...
function VisitorStaistics() {
  ...
  return (
    <>
      <div className={styles.actualTotal}>
        <div className={styles.expectTotal}>
          可预约总量<i>999999</i>人
        </div>
        <div className={styles.actualTotal}>
          {actualTotal.split("").map((item, index) => {
            return (
              <div className={styles.actualItem} key={index}>
                {item}
              </div>
            )
          })}
          <div className={styles.actualItem}>人</div>
        </div>
      </div>
      
      <div ref={echartsRef} className={styles.actualEcharts}></div>
    </>
  )
}
